...
if (!this["output"]) output = sys.puts;
var passed = 0;
var failed = 0;
...
function test_isObjectOrArray() {
    is(sync.isObjectOrArray({}), true, "isObjectOrArray detects an object");
    is(sync.isObjectOrArray([]), true, "isObjectOrArray detects an array");
    is(sync.isObjectOrArray("foo"), false, "isObjectOrArray shouldn't detect string");
    is(sync.isObjectOrArray(4), false, "isObjectOrArray shouldn't detect integer");
    is(sync.isObjectOrArray(5.5), false, "isObjectOrArray shouldn't detect float");
    is(sync.isObjectOrArray(undefined), false,
       "sync.isObjectOrArray shouldn't detect undefined");
    is(sync.isObjectOrArray(null), false, "isObjectOrArray shouldn't detect null");
}
...
function test_Command() {
    var x = new sync.Command("edit", ["foo"], 5);
    var y = new sync.Command("edit", ["foo"], 5);
    is(x instanceof sync.Command, true, "instanceof");
    is(x.equals(y), true, "equals method of Command works");
    is(sync.findValue([y], x), 0, "findValue locates Command object");
    ...
    var z = new sync.Command("edit", ["foo","bar"], 5);
    is(x.isParentOf(z), true, "check for parents");
}

function test_commandInList() {
    var x = new sync.Command("edit", ["foo"], 5);
    var y = new sync.Command("remove", ["bar"]);
    var commandList = [new sync.Command("edit", ["foo"], 5),
                       new sync.Command("remove", ["bar"])];
    is(sync.commandInList(x, commandList), true, "commandInList matches identical");
    is(sync.commandInList(x, commandList), true, "commandInList matches removes");
    is(sync.commandInList(new sync.Command("edit", ["foo"], "bar"),
			  [new sync.Command("edit", ["foo"], "bar")]),
       true, "edits match");
    is(sync.commandInList(new sync.Command("edit", ["foo"], 6), commandList), false,
       "commandInList fails differing values");
}

function test_doesConflict() {
    var x = new sync.Command("edit", ["foo"], 1);
    var y = new sync.Command("edit", ["foo"], 2);
    is(sync.doesConflict(x, y), true, "doesConflict finds identical paths with different values");

    y.path = ["bar"];
    is(sync.doesConflict(x, y), false, "doesConflict ignores mismatched paths");
    
    var a = new sync.Command("remove", ["foo"]);
    var b = new sync.Command("edit", ["foo","bar"], 42);
    is(sync.doesConflict(a, b), true, "doesConflict catches edit under remove");
    is(sync.doesConflict(b, a), true, "doesConflict catches edit under remove");
}
...
function test_reconcileWithNoConflicts() {
    var syncdata = reconcile([sync.detectUpdates(snap, replica1),
                              sync.detectUpdates(snap, replica2),
                              sync.detectUpdates(snap, replica3),
                              sync.detectUpdates(snap, replica4)]);
    ...
    sync.applyCommands(replica1, syncdata.propagations[0]);
    sync.applyCommands(replica2, syncdata.propagations[1]);
    sync.applyCommands(replica3, syncdata.propagations[2]);
    sync.applyCommands(replica4, syncdata.propagations[3]);
...
function checkUpdate(list, path, expectAction, expectValue) {
    ...
    if (sync.isObjectOrArray(expectValue))
...
function test_complexReconcileWithNoConflicts() {
    var updates = sync.detectUpdates(snapshotJSON, currentJSON);
    ...
    var otherUpdates = sync.detectUpdates(snapshotJSON, otherJSON);
    ...
    sync.applyCommands(currentJSON, syncdata.propagations[0]);
    sync.applyCommands(otherJSON, syncdata.propagations[1]);
...
function test_repeatedSyncsWithNoConflicts() {
    ...
    var syncdata = reconcile([sync.detectUpdates(originalJSON, clientJSON),
                              sync.detectUpdates(originalJSON, serverJSON)]);
    sync.applyCommands(clientJSON, syncdata.propagations[0]);
    sync.applyCommands(serverJSON, syncdata.propagations[1]);
    ...
    syncdata = reconcile([sync.detectUpdates(originalJSON, clientJSON), 
                          sync.detectUpdates(originalJSON, serverJSON)]);
    sync.applyCommands(clientJSON, syncdata.propagations[0]);
    sync.applyCommands(serverJSON, syncdata.propagations[1]);
...
function test_conflictsFromReplica() {
    var conflictList = conflictsFromReplica(new sync.Command("edit", ["f"], "b"),
                                            [new sync.Command("edit", ["f"], "b")]);
...
function test_basicConflicts() {  
    ...
    var syncdata = reconcile([sync.detectUpdates(snap, clientJSON), 
                              sync.detectUpdates(snap, serverJSON)]);
